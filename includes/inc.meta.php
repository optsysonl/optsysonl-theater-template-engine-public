 ...
 */
function get_ticketing($arr) {

    //no ticketing as defailt
    $ticketing = 0;

    //if any of these have values, they are ticketing
    if ($arr["posURL"] != "" || $arr["ticketing"] == 1) {

        // Find out if there is a specific link where the ticketing should go
        // other wise it should be the default.
        if ( $arr["posURL"] != "" ){
            $ticketing = $arr["posURL"];
        }
        else
        {
            $ticketing = 1;
    ...
    if ($arr["posType"] != '' && $arr["posType"] != "None") {
        $ticketing = 1;

    } elseif ($arr["posType"] == "None") {
        $ticketing = 0;
    }
...
/**
 * Builds a string of 48 showtime fields and am/pm fields.
 *
 * @return comma seperated string of time fields and bargina fields
 */
function showtime_fields() {
 ...
 * @return count
 */
function feature_count($arr, $screen_id) {
    $count = 0;
...
*/
function isSoldOut($soldouts, $house_id, $movie_id, $showdate, $showtime, $comment)
{
// Nothing is sold out, or don't care
    ...
    {
        $compdate = date('m/d/Y', strtotime($showdate));
        $dbshowdate = date('m/d/Y', strtotime($soldrow['showdate']));

        /*
        echo "MOVIEID :" . $soldrow['movie_id'] . " - $movie_id<br>\n";
        echo "HOUSEID :" . $soldrow['house_id'] . " - $house_id<br>\n";
        echo "SHOWDATE:" . $soldrow['showdate'] . " - $showdate<br>\n";
        echo "SHOWTIME:" . $soldrow['showtime'] . " - $showtime<br>\n";
        echo "SOLDOUT :" . $soldrow['soldout'] . "<br>\n";
        echo "DBSHOWDATE:" . $dbshowdate . " - $compdate<br><br>\n";
        */

        if ($soldrow['movie_id'] == $movie_id && $soldrow['house_id'] == $house_id && $soldrow['showtime'] == $showtime && $dbshowdate == $compdate)
            return true;
    }
