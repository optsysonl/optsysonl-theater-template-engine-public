    ...
    function setHousePopup() {
        ...
        function handlers() {
            ...
            options._search_button.on('click', function () {
                options._house_list_inner.find('.search-message').remove();
        ...
        function showSearchResults() {
            ...
            let items = options.popup.container.find('.set-house-item');
            $.ajax({
                url: '/data/location',
                type: 'post',
                dataType: "json",
                ...
                success: function(result){
                    try{
                        if(result.houses.length !== 0){
                            ...
                            $.each(result.houses, function(key, item){
                                $('#'+item.id).removeClass('hidden-item');
                            })
                        }else{
                            items.addClass('hidden-item');
                            options._house_list_inner.append('<div class="search-message" style="padding: 30px;">No results</div>');
                        }
                    }catch(error){
                        console.log(error);
                    }
        ...
        function clearSearchResults() {
            options.popup.container.find('.set-house-item').removeClass('hidden-item');
        }
