...
use templates\custom\kewgardens\app_local\Models\Screen;
...
class ShowtimesController
{
    ...
    public function showtimesByKeyword($keywords){
        $site_id = config('fxp.site_id');
        $keyword_str=MetaHelper::url_name(implode('-',$keywords));

        $cacheItem = Cache::getItem('showtimesByKeyword_site_' . $site_id.'_filter_'. $keyword_str);
        if (!Cache::isHit($cacheItem)) {
            ...
            foreach ($showdates as $a_date) {
                $cacheItem_date = Cache::getItem('showtimesByKeyword_site_' . $site_id.'_filter_'.$keyword_str.'_date_'.$a_date);
                if (!Cache::isHit($cacheItem_date)) {
                    $showtimes_date = [];

                    $showtimes_results = $localScreen->getShowtimesByMovieName($a_date, $keywords);
                    foreach ($showtimes_results as $result) {
                        // Set timezone of house
                        TimeZoneHelper::setTimezone($result->timezone);
                        $now = DateHelper::now();
