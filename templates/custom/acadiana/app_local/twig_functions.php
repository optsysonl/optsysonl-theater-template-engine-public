...
function getHouseTimesFunction($showdates, $house_id, $movies){
    ...
    if (!Cache::isHit($cacheItem)) {
        ...
        $period = new DatePeriod(
            ...
            new DateTime($this_house_dates[0])
        );
        foreach($period as $key => $value){
            $cur_date = $value->format('Y-m-d');
            $this_house_dates[] = $cur_date;
            $this_house_data[$cur_date]['movies'] = [];
        }
        usort($this_house_dates, function($date1, $date2){
